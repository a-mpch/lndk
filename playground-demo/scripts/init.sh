...
sendFundingTransaction() {
  ...
  local addresses=($LND1_ADDRESS $ECLAIR1_ADDRESS $ECLAIR2_ADDRESS $CLN1_ADDRESS)
...
openChannel() {
  ...
  # Open a channel between eclair1 and cln1.
  echo "Opening channel between eclair1 and cln1"
  waitFor eclair1 connect --uri=$CLN1_NODE_URI
  waitFor eclair1 open --nodeId=$CLN1_PUBKEY --fundingSatoshis=10000000 --pushMsat5000000

  # Open a channel between eclair2 and cln1.
  echo "Opening channel between eclair1 and cln1"
  waitFor eclair2 connect --uri=$CLN1_NODE_URI
  waitFor eclair2 open --nodeId=$CLN1_PUBKEY --fundingSatoshis=10000000 --pushMsat5000000
...
copyMacaroonAndTlsLocally() {
  ...
  docker cp playground-demo-lnd1-1:/root/.lnd/data/chain/bitcoin/regtest/admin.macaroon $DIR/../../.configs/playground/lnd-secrets/admin.macaroon
  docker cp playground-demo-lnd1-1:/root/.lnd/tls.cert $DIR/../../.configs/playground/lnd-secrets/tls.cert
}
