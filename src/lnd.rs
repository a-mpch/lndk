...
const SEED_KEY_FAMILY: u32 = 4;
const SEED_KEY_INDEX: u32 = 4444;
...
pub async fn build_seed_from_lnd_node(signer: &mut impl MessageSigner) -> Result<[u8; 32], ()> {
    ...
    let key_loc = KeyLocator {
        key_family: SEED_KEY_FAMILY as i32,
        key_index: SEED_KEY_INDEX as i32,
    };

    let msg = "LNDK:DO NOT SIGN THIS MANUALLY:SEED";

    let signature = signer
        .sign_message(msg.as_bytes(), key_loc, true, true)
        .await?;
